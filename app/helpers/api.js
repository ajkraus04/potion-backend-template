  ...
  try {
    ...
    if (type === parameterTypes.sqs) {
      if (event.Records != null && event.Records[0] != null) {
        data =
          typeof event.Records[0].body !== "object"
            ? JSON.parse(event.Records[0].body)
            : event.Records[0].body;
      } else {
        data =
          typeof event.sqsBody !== "object" && event.sqsBody != null
            ? JSON.parse(event.sqsBody)
            : event.sqsBody != null
              ? event.sqsBody
              : {};
      }
    ...
    event.validData = {};
    if (validator != null) {
      event.validData = await validator.validateAsync(data, {
